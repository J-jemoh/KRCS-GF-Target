...
class QPMMController extends Controller
{
    //
    public function uploadQpmm(Request $request){

        $request->validate([
            'qpmmF'=>'required|mimes:csv',

        ]);
        if ($request->file('qpmmF')->isValid()) {
            $file=$request->file('qpmmF');
            ...
            if (count($csvData) > 0 && is_array($csvData[0])) {
                ...
                return redirect()->route('admin.qpmm')->with('error', 'CSV file is empty.');
            }
            ...
            // Process CSV data and save to database
                    foreach ($csvData as $row) {
                        ...
                        // dd($data);
                        $existingRecord = QPMM::where('sno',$data['sno'])
             ...
             return redirect()->route('admin.qpmm')->with('success', 'Your QPMM file has been uploaded successfully.');
        }
        return redirect()->route('admin.qpmm')->with('error', 'Invalid file.');
